...
send_rexmit_request( gpointer data)
{
	...
	/* 再送メッセージに情報をセットする */
	/* 再送要求の最初と最後にはシーケンス番号が入っているだけなので、１つのメッセージの再送要求のみとなっているので注意 */
	...
	/* 再度、再送メッセージをスケジュールする？？？なぜ？ */
	/* たぶん、再送受信でremove_msg_rexmit()まで再送を要求するのだと思う */
...
schedule_rexmit_request(struct node_info* node, seqno_t seq, int delay)    
{
	...
	/* delayパラメータが０の場合は、ランダムなタイマー値を計算する */
	if (delay == 0) {
	...
	/* 再送メッセージに再送依頼するシーケンス番号と、ノードをセットする */
	ri->seq = seq;
	ri->node = node;
	
	/* ディレイタイマーを仕掛けて、作成した再送メッセージを送信する */
	...
	if (rexmit_hash_table == NULL){
		/* 再送メッセージハッシュテーブルが未作成の場合は作成する */
		init_rexmit_hash_table();
	}
	/* 再送メッセージハッシュテーブルに、タイマー情報を登録する */
...
request_msg_rexmit(struct node_info *node, seqno_t lowseq,	seqno_t hiseq)
{
	...
	int i;
	/* ロストした最小シーケンス数から最大シー件数数までの再送要求を送信する */
...
remove_msg_rexmit(struct node_info *node, seqno_t seq)
{
	...
	/* シーケンス番号・ノード情報を元に再送メッセージハッシュテーブルを検索する */
	...
	if (value == NULL){
		...
		/* 存在した場合は、タイマーをキャンセルし、再送メッセージハッシュテーブルから情報を削除する */
