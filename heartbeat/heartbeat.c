...
free_one_hist_slot(struct msg_xmit_hist* hist, int slot )
{
	...
	if (msg){
		/* メッセージが未解放の場合 */
		/* 履歴中の現在有効な最小シーケンス番号を対象スロットのシーケンス番号でセットする */
		hist->lowseq = hist->seqnos[slot];
		/* 対象スロットのメッセージポインタをNULLクリア */
		hist->msgq[slot] = NULL;
		if (!msg) {
			/* 無駄ではない？かな? msgがNULL以外しか入らないので */
			cl_log(LOG_CRIT,
	      ...
	      TIME_T msgtime, seqno_t seqno, const char * iface, struct ha_msg * msg)
{
	...
	if (ackseq == fromnode->track.ackseq){
		/* 受信ACKメッセージのシーケンス番号と送信元ノード・シーケンス記録情報のACKシーケンス番号が一致する場合は */
		/* 処理済みなので処理しない */
		/*dup message*/
		goto out;
	}
	...
	/* 送信元ノード・シーケンス記録情報のACKシーケンス番号を受信ACKシーケンス番号でセットする */
	...
	&&	STRNCMP_CONST(hist->lowest_acknode->status,DEADSTATUS)==0){
		 ...
		 */
		/* 履歴情報のlowest_acknodeがNULLでなくて、そのノードがDEADSTATUSになった場合は、 */
		/* 履歴情報のlowest_acknodeをNULLクリアする */
		hist->lowest_acknode = NULL;
	}
...
update_ackseq(seqno_t new_ackseq) 
{
	...
	if (new_ackseq <= old_ackseq){
		/* 履歴情報のACKシーケンス番号以下のACKシーケンス番号が来た場合には処理不要 */
		return;
	}
	
	/* 履歴情報のACKシーケンス番号を受信したACKシーケンス番号でセット */
...
send_ack(struct node_info* thisnode, seqno_t seq)
{
	...
	/* メッセージタイプ(ACKMSG),宛先ノード名、ACK対象シーケンス番号をセットして、ACKメッセージを送信する */
...
send_ack_if_needed(struct node_info* thisnode, seqno_t seq)
{
	...
	if ( (fm_seq != 0 && seq > fm_seq) ||
		...
		/*no need to send ACK */
		/* first_missing_seqが０でなくて、first_missing_seqaが受信シーケンス番号よりも小さい時（再送ではない） */
		/* シーケンス番号が、ACK送信トリガー値でない場合は、ACK送信せずにリターン */
		return;
	}	
	/* ACKを送信する */
...
/* ACK送信の必要性を判定して、必要があればACKメッセージを送信する */
static void
send_ack_if_necessary(const struct ha_msg* m)
{
	...
	if (thisnode == NULL){
		cl_log(LOG_ERR, "node %s not found "
		       "bad message",
		       fromnode);
		/* クラスタ構成に存在しないノードからのメッセージの場合は処理しない */
		return;		
	}
	/* ACK送信判定処理 */
...
process_clustermsg(struct ha_msg* msg, struct link* lnk)
{
	 ...
	 */
	/* 再送依頼で再送されたメッセージの場合は、update_client_status_msg_list()を処理する */
	if (missing_packet){		
...
main(int argc, char * argv[], char **envp)
{
	...
	hb_register_msg_callback(T_REXMIT,	HBDoMsg_T_REXMIT);	/* 再送依頼メッセージの受信ハンドラ */
...
reset_seqtrack(struct node_info *n)
{
	...
	for (i = 0; i < t->nmissing; ++i) {
		...
		if (seq == NOSEQUENCE)
			/* 使われていないエリアは処理しない */
			continue;
		/* 再送メッセージハッシュテーブルからメッセージを削除して、再送タイマーを停止する */
...
{
	struct seqtrack *	t = &thisnode->track;
	const char *		cseq = ha_msg_value(msg, F_SEQ);	/* 受信メッセージのシーケンス番号を取り出す */
	...
	&&	strncmp(type, NOSEQ_PREFIX, sizeof(NOSEQ_PREFIX)-1) ==	0) {
		...
		if (strcasecmp(type, T_NAKREXMIT) == 0) {
			/* 再送依頼メッセージのNAKREXMIT(再送依頼したメッセージが存在しない）を受信した場合 */
	...
	/* 宛先のuuidもしくは、nodenameから自ノードが宛先かどうか判定する */
	/* IsToUS=TRUEは自ノード宛先：FALSEはその他のノード */
	...
	if (gen < t->generation) {
		...
		if (gen == t->generation && gen > 0) {
			...
			/* DEADステータスのノードからのSTATUSメッセージの場合 */
			if (strcmp(thisnode->status, DEADSTATUS) == 0) {
				/* Is this stale data? */
				if (seq <= thisnode->status_seqno) {
					/* 今回受信したシーケンス番号が、古いシーケンス番号の場合はDROPITでリターン */
					return DROPIT;
				}
			...
			/* STATUSメッセージを受信した場合で、保持していたノードのgeneration情報よりも受信したgenerationの方が大きい場合 */
			/* ノードのリスタートフラグをセットする */
			isrestart = 1;
			if (t->generation > 0) {
				/* リスタートログを出力する */
		...
		/* 受信したSTATUSメッセージのgenerationでノードのシーケンス情報のgenerationを更新する */
		t->generation = gen;
	}

	/* Is this packet in sequence? */
	if (t->last_seq == NOSEQUENCE || seq == (t->last_seq+1)) {
		/* last_seqがNOSEQUENCE もしくは、受信したシーケンス番号が最後のシーケンス番号+1(正しいシーケンス)の時 */
		/* 最後のシーケンス番号と、最後のインターフェース情報をセットする */
		t->last_seq = seq;
		t->last_iface = iface;
		/* ACKメッセージの送信の必要性を判定して、必要があればACKメッセージを送信する */
		send_ack_if_necessary(msg);
		return (IsToUs ? KEEPIT : DROPIT);	/* 宛先判定からDROPIT,KEEPITを返す */
	}else if (seq == t->last_seq) {
		/* 最後に受信したのと同じシーケンス番号のメッセージを受信した場合（再度同じメッセージを受信した場合)
	 ...
	 */
	if (seq > t->last_seq) {
		/* 対象ノードから受信した最後のシーケンスよりも、今回のシーケンスが大きい時（パケットがロスト） */
		seqno_t	k;
		seqno_t	nlost;
		/* ロスト数を計算してログに出力する */
		...
		if (t->first_missing_seq == 0
			...
			/* 対象ノードのfirst_missing_seqが初期値（０）か、最後のシーケンス＋１よりも大きい時は */
			/* 対象ノードのfirst_missing_seqに最後のシーケンス＋１をセットする */
			t->first_missing_seq = t -> last_seq +1;				
		} 

		/* ロストしたシーケンス番号のエリアを全て処理する */
		for(k = t->last_seq+1; k < seq; ++k) {
			if (t->nmissing < MAXMISSING-1) {
				/* ロスト数が最大数を超えていない場合は、ロスト数のエリアに、ロストしたシーケンス番号をセットする */
				...
				for (j=0; j < MAXMISSING; ++j) {
					if (t->seqmissing[j] == NOSEQUENCE) {
						/* 未使用のエリアがあればインデックスをセットする */
						minmatch = j;
						break;
					}
					if (minmatch < 0
					|| t->seqmissing[j] < minseq) {
						/* minmatchが初期値か、minseqが直前のエリアよりも小さい場合 */
						/* 候補エリアのインデックスにループインデックスをセットする */
						/* ※これによって、未使用エリアがない場合でも、最小のシーケンス番号を保持しているエリアが利用される */
						minmatch = j;
						minseq = t->seqmissing[j];
					}
				}
				/* 検索したロスト数のエリアに、ロストしたシーケンス番号をセットする */
...
is_lost_packet(struct node_info * thisnode, seqno_t seq)
{
	...
	int			j;
	int			ret = 0;
	/* 受信したシーケンスから、ノードのロストパケットとして処理されているメッセージを受信した場合に、シーケンス記録情報を */
	/* 更新する */
	/* 対象ノードのロスト件数を全て処理する */
	for (j=0; j < t->nmissing; ++j) {
		/* Is this one of our missing packets? */
		if (seq == t->seqmissing[j]) {
			/* 対象ノードから受信したシーケンス番号が、ロスト管理しているシーケンス番号と一致する場合 */
			/* 再送メッセージハッシュテーブルからの情報削除(再送タイマーも停止されるので注意) */
			...
			&&	t->seqmissing[t->nmissing-1] == NOSEQUENCE) {
				t->nmissing --;
				/* 管理配列に未使用のエリアがある場合はロスト件数をこの時点で正確な値にセットする */
			}
			if (t->nmissing == 0) {
				/* ロスト件数が０（未使用エリアのみ）となった場合は、ログを出力する */
				cl_log(LOG_INFO, "No pkts missing from %s!"
				,	thisnode->nodename);
				/* ロスト件数がないので、ロストしたシーケンス番号の最小値を０にリセットする */
				t->first_missing_seq = 0;
			}
			ret = 1;
			goto out;
 ...
 out:
	...
	if (ret && seq == t->first_missing_seq){
		/* 管理エリアからロストしたパケットを処理した場合で、処理したシーケンス番号が、ロストしたシーケンス番号の最小値の場合 */
		...
		seqno_t ack_seq;
		/* ロストしたシーケンス番号の最小値をリセットする */
		t->first_missing_seq = 0;
		for (j=0; j < t->nmissing; ++j) {
			/* ロストしたシーケンス番号の最小値を残っている記録情報から再セットする */
			if (t->seqmissing[j] != NOSEQUENCE){
				if (t->first_missing_seq == 0 ||
		...
		if (t->first_missing_seq == 0){
			/* first_missing_seqが０（ロストはもうない)場合は、最後の受信シーケンス番号をセット */
			lastseq_to_ack = t->last_seq;			
		}else {
			/* first_missing_seqが０以外の場合は、first_missing_seq－１をセット */
			lastseq_to_ack = t->first_missing_seq - 1 ;
		}
		
		/* lastseq_to_ackを１０で割ったあまりを算出 */
...
/* 自ノードからの送信シーケンスメッセージの履歴情報の初期化処理 */
...
,	seqno_t seq)
{
	...
	/* 最終スロット番号＋１でインデックスを計算 */
	slot = hist->lastmsg+1;
	if (slot >= MAXMSGHIST) {
		/* 最大履歴数を超えた場合はインデックスを０にリセット */
		/* 初期値　hist->lastmsgは、MAXMSGHIST-1にセット(init_xmit_hist内)されているので、０から開始されることになる */
		slot = 0;
	}
	/* 最大シーケンス番号を送信シーケンス番号でセット */
	hist->hiseq = seq;
	/* 利用インデックスのメッセージのポインタを取り出す */
	slotmsg = hist->msgq[slot];
	/* Throw away old packet in this slot */
	/* 履歴エリアが１周した後は、未解放のエリアでも解放して利用する */
	if (slotmsg != NULL) {
		/* メッセージが存在する場合 */
		/* Lowseq is less than the lowest recorded seqno */
		/* 最小シーケンス番号を、履歴の対象インデックスのシーケンス番号でセット */
		hist->lowseq = hist->seqnos[slot];
		/* 対象履歴メッセージ本体エリアをNULLクリア */
		hist->msgq[slot] = NULL;
		if (!slotmsg) {
			/* これも無駄でないか？slotmsgがNULLでない場合しか入らない */
			cl_log(LOG_CRIT
	...
	/* インデックスの履歴メッセージエリアにメッセージをセット */
	hist->msgq[slot] = msg;
	/* インデックスの履歴シーケンス番号エリアにメッセージのシーケンス番号をセット */	
	hist->seqnos[slot] = seq;
	/* インデックスの履歴再送エリアを０でクリア */
	...
	&&	live_node_count > 1) {
		int priority = 0;
		/* フロー制御が有効で、構成ノード数が２ノード以上の場合は、履歴の保持状態からログを出力する */
...
process_rexmit(struct msg_xmit_hist * hist, struct ha_msg* msg)
{
	...
	/* 再送要求の最初と最後のシーケンス番号を取り出す */
	/* 実際は、最初と最後は同じ値が入っているので１つづずの再送要求が着ている */
	...
	for (thisseq = fseq; thisseq <= lseq; ++thisseq) {
		...
		/* メッセージ履歴の最新のインデックスから、逆順に履歴を全て処理する */
		...
		;	!foundit && msgslot != (firstslot+1); --msgslot) {
			...
			if (hist->seqnos[msgslot] != thisseq) {
				/* シーケンス番号が再送のシーケンスと一致しないスロットも処理しない */
				continue;
			}
			...
			/* メッセージ履歴の該当スロットから、再送メッセージを取り出してメッセージに変換する */
...
/* 再送依頼メッセージが存在しない場合の処理(NAXREXMITを送信) */
static void
