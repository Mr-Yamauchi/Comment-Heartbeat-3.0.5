...
typedef struct llm_node_s {
	char	nodename[NODEIDSIZE];		/* ノード名 */
	char	status[STATUSSIZE];			/* ステータス */
	int	uptime;
...
typedef struct llm_info_s { 
	int	   nodecount;				/* ノード数 */
	int	   myindex;					/* 自ノードインデックス */
	llm_node_t nodes[MAXNODE];		/* ノードステータス情報 */
} llm_info_t;
...
typedef struct ccm_info_s {
	llm_info_t 	llm;	/*  low level membership info */	/* メンバー情報 */
...
#define ccm_hiproto	  ccm_proto.com_hiproto
	...
	char		cluster[PATH_MAX];							/* クラスター名 : デフォルトlinux_ha */
