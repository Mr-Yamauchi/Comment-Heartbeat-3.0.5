...
/* global_info(ccm_info_t)をセット */
static int
set_llm_from_heartbeat(ll_cluster_t* llc, ccm_info_t* info){
	llm_info_t*	llm = &info->llm;
	struct llc_ops* ops = llc->llc_ops; /* Heartbeatの操作ポインタをセット */
	const char*	status;
	const char*	node;
	const char*	mynode = ops->get_mynodeid(llc);	/* ノードIDを取得 */
	const char*	cluster;
...
*/
	...
	while((node = ops->nextnode(llc)) != NULL) {		
		...
		ccm_debug2(LOG_DEBUG, "Cluster node: %s: status: %s", node,
			       status);
		/* メンバー情報に対象ノードのステータスなどの取得情報を追加する */
...
ccm_initialize()
{
	...
	*/
	...
	/* ccmret情報にglobal_info,Heartbeat接続ハンドルをセット */
